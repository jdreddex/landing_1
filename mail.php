...
if($_POST['name']){ // заносим в массив значение полей, их может быть больше
  ...
  mail("admin@jdreddex.byethost22.com", "Заказ Звонка".$_SERVER['HTTP_REFERER'],  
  $znach[1]." ". 
  $znach[2]); // письмо на свой электронный ящик, измените на свой email
}
if($_POST['name']){ // заносим в массив значение полей, их может быть больше
  $znach = array(
    1 => $_POST['name'],
  ...
  mail("john.reddex@gmail.com", "Заказ Звонка".$_SERVER['HTTP_REFERER'],  
  $znach[1]." ". 
  $znach[2]); // письмо на свой электронный ящик, измените на свой email
}
Header("Refresh: 6; URL=".$_SERVER['HTTP_REFERER']); // спустя 3 секунд человек будет возвращён на предыдущий URL
?>
